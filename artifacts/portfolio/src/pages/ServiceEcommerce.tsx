...
import { ArrowRight, ShoppingCart, CreditCard, Package, BarChart3, Search, Shield, RefreshCw, Zap, Star } from 'lucide-react';
...
const FEATURES = [
  { icon: ShoppingCart,title: 'Custom Storefronts',         desc: 'Shopify 2.0, WooCommerce and headless e-commerce built to your brand — not a theme out of the box.' },
  { icon: CreditCard, title: 'Payment Integration',         desc: 'Stripe, PayPal, Klarna, Apple Pay and local gateways. We handle the whole checkout experience.' },
  { icon: Package,    title: 'Product & Inventory',         desc: 'Bulk product import, variants, bundles, digital downloads and real-time inventory sync with your warehouse.' },
  { icon: Search,     title: 'E-Commerce SEO & AEO',        desc: 'Product schema, review markup, faceted navigation and crawlable architecture for Google and AI search.' },
  { icon: BarChart3,  title: 'Analytics & Attribution',     desc: 'GA4, Meta Pixel, server-side tracking and custom dashboards so you know exactly where revenue comes from.' },
  { icon: Shield,     title: 'Fraud Prevention',            desc: 'Stripe Radar, address validation and order-risk scoring to protect your revenue from chargebacks.' },
  { icon: Zap,        title: 'Performance Optimisation',    desc: 'Sub-2s page loads with lazy loading, CDN, image compression and Core Web Vitals tuned for conversion.' },
  { icon: RefreshCw,  title: 'Migration & Re-platforming',  desc: 'Moving from Magento, BigCommerce or an old Shopify theme? We migrate products, orders and SEO signals intact.' },
];

const PACKAGES = [
  { name: 'Starter Store',    price: '$399',  delivery: '7 days',  desc: 'Up to 50 products, payment gateway and basic SEO', popular: false },
  { name: 'Growth Store',     price: '$999',  delivery: '14 days', desc: 'Custom theme, AEO schema, multi-currency and analytics', popular: true },
  { name: 'Enterprise Store', price: '$2,199',delivery: '28 days', desc: 'Headless commerce, multi-warehouse and ERP integration', popular: false },
];

const REVIEWS = [
  { name: 'Bilal H.', stars: 5, text: 'Our WooCommerce store was converted to Shopify and sales jumped 60% in the first quarter. The AEO schema alone was worth it.' },
  { name: 'Emma C.', stars: 5, text: 'Deverdex set up Klarna and Apple Pay in a day. Checkout abandonment dropped immediately. Brilliant team.' },
  { name: 'Ahmed N.', stars: 5, text: 'The product migration was flawless — 3,000 SKUs, all reviews preserved and no broken URLs. Exceptional work.' },
];
      ...
      <section ref={heroRef} className="pt-28 pb-32 relative overflow-hidden border-b border-white/5 min-h-[80vh] flex items-center">
        ...
        <motion.div style={{ y: heroY, opacity: heroOpacity }} className="container mx-auto px-6 md:px-12 relative z-10 max-w-5xl">
          ...
          <h1 className="font-bold leading-[1.08] mb-8 max-w-4xl">
            {[
              { text: 'Online stores that', color: 'text-white', delay: 0.1 },
              { text: 'turn browsers', color: 'text-white', delay: 0.28 },
              { text: 'into', color: 'text-white', delay: 0.42 },
              { text: 'buyers.', color: 'text-[#00DCB9]', delay: 0.54 },
            ].map((chunk, i) => (
          ...
          <motion.p className="text-lg text-[#8A9BB8] leading-relaxed max-w-xl mb-10"
            ...
            Custom{' '}
            <span className="text-white underline decoration-white/20 underline-offset-2">Shopify</span>,{' '}
            <span className="text-white underline decoration-white/20 underline-offset-2">WooCommerce</span> and headless stores
            with AEO schema, conversion-optimised checkout and full analytics.
          </motion.p>
        ...
        <div className="container mx-auto px-6 md:px-12 max-w-6xl relative z-10">
          <motion.div {...fadeUp(0)} className="mb-4">
            ...
            <h2 className="text-4xl md:text-6xl font-bold leading-tight max-w-3xl">
              <span className="text-white">E-commerce built to</span><br />
              <span className="text-[#00DCB9]">rank, convert</span><span className="text-white"> and </span><span className="text-[#FF6B35]">scale.</span>
            </h2>
          </motion.div>
          <motion.div {...fadeUp(0.1)}>
            ...
            <p className="text-[#8A9BB8] text-lg leading-relaxed max-w-2xl">From product catalogue to checkout flow — we handle every aspect of your online store.</p>
          </motion.div>
      ...
      <section className="py-28 bg-[#06090F] border-b border-white/5 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 md:px-12 max-w-5xl relative z-10">
          <motion.div {...fadeUp(0)} className="mb-14 text-center">
            ...
            <h2 className="text-4xl md:text-5xl font-bold text-white">Pick your store tier.</h2>
          </motion.div>
      ...
      <section className="py-24 border-b border-white/5">
        <div className="container mx-auto px-6 md:px-12 max-w-6xl">
          <motion.div {...fadeUp(0)} className="mb-12 text-center">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-white">Trusted by online retailers worldwide.</h2>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {REVIEWS.map((r, i) => (
      ...
      <section className="py-28 relative overflow-hidden">
        ...
        <motion.div className="container mx-auto px-6 md:px-12 max-w-2xl relative z-10 text-center" {...fadeUp(0)}>
          <div className="font-mono text-xs text-[#00DCB9] uppercase tracking-widest mb-6 flex items-center justify-center gap-2">
            <span className="w-5 h-px bg-[#00DCB9]/50" /> START SELLING <span className="w-5 h-px bg-[#00DCB9]/50" />
          </div>
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-6 leading-tight">Ready to launch<br /><span className="text-[#00DCB9]">your online store?</span></h2>
          <p className="text-[#6A7B98] text-lg mb-10">Tell us about your products — we'll respond within 24 hours.</p>
