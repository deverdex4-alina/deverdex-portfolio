...
import { ArrowRight, Smartphone, Zap, Bell, MapPin, Shield, ShoppingBag, BarChart3, RefreshCw, Star } from 'lucide-react';
...
const FEATURES = [
  { icon: Smartphone, title: 'React Native & Expo',         desc: 'Cross-platform apps for iOS and Android from a single TypeScript codebase — no compromise on feel or performance.' },
  { icon: Zap,        title: 'Native Performance',          desc: 'Native modules, Reanimated 3, and Skia rendering for fluid 60fps animations and interactions.' },
  { icon: Bell,       title: 'Push Notifications',          desc: 'Expo Notifications or Firebase FCM — rich push alerts, deep linking and notification analytics.' },
  { icon: MapPin,     title: 'Maps & Location',             desc: 'Real-time location tracking, geofencing, map overlays and route planning with React Native Maps.' },
  { icon: Shield,     title: 'Auth & Biometrics',           desc: 'Secure login flows with OAuth, Face ID, Touch ID and device-level credential storage.' },
  { icon: ShoppingBag,title: 'In-App Purchases',            desc: 'RevenueCat and StoreKit integration for subscriptions, consumables and paywall flows on both platforms.' },
  { icon: BarChart3,  title: 'Analytics & Crash Reporting', desc: 'Sentry, Mixpanel or Amplitude integrated from day one so you ship with observability, not guesswork.' },
  { icon: RefreshCw,  title: 'OTA Updates & CI/CD',         desc: 'Expo EAS Build and Submit pipelines for automated App Store and Play Store delivery without manual uploads.' },
];

const PACKAGES = [
  { name: 'MVP App',        price: '$999',  delivery: '14 days', desc: 'Core screens, auth, one API integration and App Store submission', popular: false },
  { name: 'Full Mobile App',price: '$1,999',delivery: '28 days', desc: 'Custom UI, push notifications, payments and in-app analytics', popular: true },
  { name: 'Enterprise App', price: '$3,499',delivery: '45 days', desc: 'Offline support, biometrics, complex data sync and white-labelling', popular: false },
];

const REVIEWS = [
  { name: 'Natalie B.', stars: 5, text: 'Our fitness app launched in 6 weeks and hit 4.8 stars on the App Store. The animations are genuinely beautiful.' },
  { name: 'Daniel M.', stars: 5, text: 'Deverdex built both iOS and Android versions simultaneously. The code quality is extraordinary for the price.' },
  { name: 'Kezia R.', stars: 5, text: 'We shipped OTA updates after launch with zero disruption to users. The EAS setup they built just works.' },
];

export function ServiceMobileApps() {
    ...
    <div className="w-full bg-[#080E14]">
      <section ref={heroRef} className="pt-28 pb-32 relative overflow-hidden border-b border-white/5 min-h-[80vh] flex items-center">
        ...
        <motion.div style={{ y: heroY, opacity: heroOpacity }} className="container mx-auto px-6 md:px-12 relative z-10 max-w-5xl">
          ...
          <h1 className="font-bold leading-[1.08] mb-8 max-w-4xl">
            {[
              { text: 'Mobile apps users', color: 'text-white', delay: 0.1 },
              { text: 'actually', color: 'text-[#00DCB9]', delay: 0.3 },
              { text: 'open', color: 'text-[#FF6B35]', delay: 0.42 },
              { text: '— and keep.', color: 'text-white', delay: 0.55 },
            ].map((chunk, i) => (
          ...
          <motion.p className="text-lg text-[#8A9BB8] leading-relaxed max-w-xl mb-10"
            ...
            <span className="text-white underline decoration-white/20 underline-offset-2">React Native</span> and{' '}
            <span className="text-white underline decoration-white/20 underline-offset-2">Expo</span> apps for iOS and Android —
        ...
        <div className="container mx-auto px-6 md:px-12 max-w-6xl relative z-10">
          <motion.div {...fadeUp(0)} className="mb-4">
            ...
            <h2 className="text-4xl md:text-6xl font-bold leading-tight max-w-3xl">
              <span className="text-white">Everything your app needs</span><br />
              <span className="text-[#00DCB9]">to launch </span><span className="text-[#FF6B35]">and grow.</span>
            </h2>
          </motion.div>
          <motion.div {...fadeUp(0.1)}>
            ...
            <p className="text-[#8A9BB8] text-lg leading-relaxed max-w-2xl">From MVP to production — with native performance, real-time features and App Store submission handled for you.</p>
          </motion.div>
      ...
      <section className="py-28 bg-[#06090F] border-b border-white/5 relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 md:px-12 max-w-5xl relative z-10">
          <motion.div {...fadeUp(0)} className="mb-14 text-center">
            ...
            <h2 className="text-4xl md:text-5xl font-bold text-white">Launch at your pace.</h2>
          </motion.div>
      ...
      <section className="py-24 border-b border-white/5">
        <div className="container mx-auto px-6 md:px-12 max-w-6xl">
          <motion.div {...fadeUp(0)} className="mb-12 text-center">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-white">Loved by app founders.</h2>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {REVIEWS.map((r, i) => (
      ...
      <section className="py-28 relative overflow-hidden">
        ...
        <motion.div className="container mx-auto px-6 md:px-12 max-w-2xl relative z-10 text-center" {...fadeUp(0)}>
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-6 leading-tight">Ready to build<br /><span className="text-[#00DCB9]">your mobile app?</span></h2>
          <p className="text-[#6A7B98] text-lg mb-10">Tell us about your idea — we'll respond within 24 hours.</p>
